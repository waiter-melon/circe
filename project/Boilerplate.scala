...
object Boilerplate {
  ...
  object GenSumDecoders extends Template {
    ...
    def content(tv: TemplateVals): String = {
      ...
      block"""
        ...
        |        val fs = f.asInstanceOf[Json.JObject].o.fields
        |        if (fs.size == 1) {
        |          val fv = fs.head
        ...
        -   * @group Sum
        -   */
        -  final def forSum$arity[${`A..N`}, Target](typeField: Option[String])($memberNames)(implicit
        -    $instances,
        -    $evs
  ...
  object GenSumEncoders extends Template {
    ...
    def content(tv: TemplateVals): String = {
      ...
      val findConstructor = synTypes.init.foldRight(
        s"deconstruct${ synTypes.last }.lift(a).map(t => (name${ synTypes.last }, encode${ synTypes.last }.encodeObject(t)))"
      ) {
        case (tpe, acc) => s"deconstruct$tpe.lift(a).map(t => (name$tpe, encode$tpe.encodeObject(t))).orElse($acc)"
      }
      ...
      block"""
        ...
        -   * @group Sum
        -   */
        -  final def forSum$arity[${`A..N`}, Source](typeField: Option[String])($memberNames)($deconstructors)(implicit
        -    $instances
        -  ): ObjectEncoder[Source] = typeField match {
        -    case Some(tf) => new ObjectEncoder[Source] {
        -      final def encodeObject(a: Source): JsonObject = $findConstructor match {
        -        case Some((name, o)) => o.add(tf, Json.fromString(name))
